...
class FaceController:
    ...
    def index_faces(body: IndexFacesRequest) -> IndexFacesResponse:
        ...
        sucesso, puladas, falhas, resultados = service.indexar_bucket_s3(body.s3_folder, body.collection_id)
        results = [
            FaceIndexResult(
                filename=r['filename'],
    ...
    @staticmethod
    async def search_face(file: UploadFile, body: SearchFaceRequest, user_id: int = None, event_id: int = None) -> SearchFaceResponse:
        service = RekognitionService()
        image_bytes = await file.read()

        try:
            face_detected, confidence, matches = service.buscar_rosto_por_imagem(
                image_bytes, body.collection_id, body.threshold, body.max_faces
            )
        except Exception as e:
            if event_id:
                FaceController._save_face_search(user_id=user_id, event_id=event_id, collection_id=body.collection_id,
                                                  threshold=body.threshold, max_faces=body.max_faces,
                                                  face_detected=False, face_confidence=None, matches_count=0)
            return SearchFaceResponse(success=False, face_detected=False, face_confidence=None, matches=[], message=str(e))
        ...
        if not face_detected:
            if event_id:
                FaceController._save_face_search(user_id=user_id, event_id=event_id, collection_id=body.collection_id,
                                                  threshold=body.threshold, max_faces=body.max_faces,
                                                  face_detected=False, face_confidence=None, matches_count=0)
            return SearchFaceResponse(success=False, face_detected=False, face_confidence=None, matches=[], message="Nenhuma face detectada na imagem")

        face_matches = [
            FaceMatch(name=m['name'], similarity=m['similarity'], face_id=m['face_id'], image_url=m['image_url'])
            for m in matches
        ]
        ...
        if event_id:
            FaceController._save_face_search(user_id=user_id, event_id=event_id, collection_id=body.collection_id,
                                              threshold=body.threshold, max_faces=body.max_faces,
                                              face_detected=True, face_confidence=confidence, matches_count=matches_count)

        if not face_matches:
            return SearchFaceResponse(success=True, face_detected=True, face_confidence=confidence, matches=[],
                                       message=f"Face detectada mas nenhuma correspondência encontrada com threshold de {body.threshold}%")

        return SearchFaceResponse(success=True, face_detected=True, face_confidence=confidence,
                                   matches=face_matches, message=f"Encontradas {matches_count} correspondência(s)")
    ...
    def search_face_s3(body: SearchFaceS3Request) -> SearchFaceResponse:
        ...
        try:
            face_detected, confidence, matches = service.buscar_rosto_s3(
                body.s3_key, body.collection_id, body.threshold, body.max_faces
            )
        except Exception as e:
            return SearchFaceResponse(success=False, face_detected=False, face_confidence=None, matches=[], message=str(e))
        ...
        return SearchFaceResponse(success=True, face_detected=True, face_confidence=confidence,
                                   matches=face_matches, message=f"Encontradas {len(face_matches)} correspondência(s)")
    ...
    def list_faces(body: ListFacesRequest) -> ListFacesResponse:
        ...
        face_infos = [
            FaceInfo(face_id=f['face_id'], external_image_id=f['external_image_id'], confidence=f['confidence'])
            for f in faces
        ]
        return ListFacesResponse(success=True, total_faces=len(face_infos), faces=face_infos,
    ...
    @staticmethod
    def _save_face_search(user_id: int = None, event_id: int = None, collection_id: str = None,
                           threshold: float = None, max_faces: int = None, face_detected: bool = None,
                           face_confidence: float = None, matches_count: int = 0):
        if not event_id:
            return
