...
class RekognitionService:
    ...
    def obter_faces_indexadas(self, collection_id: str = "meu_banco_de_rostos") -> Set[str]:
        try:
            ...
            while True:
                if next_token:
                    response = self.rekognition.list_faces(CollectionId=collection_id, MaxResults=4096, NextToken=next_token)
                else:
                    response = self.rekognition.list_faces(CollectionId=collection_id, MaxResults=4096)
    ...
    def listar_faces(self, collection_id: str = "meu_banco_de_rostos", max_results: int = 4096) -> List[Dict]:
        try:
            ...
            while True:
                if next_token:
                    response = self.rekognition.list_faces(CollectionId=collection_id, MaxResults=max_results, NextToken=next_token)
                else:
                    response = self.rekognition.list_faces(CollectionId=collection_id, MaxResults=max_results)
                for face in response.get('Faces', []):
                    faces.append({
    ...
    def indexar_bucket_s3(self, s3_folder: str = "rostos/", collection_id: str = "meu_banco_de_rostos") -> Tuple[int, int, int, List[Dict]]:
        try:
                ...
                try:
                    ...
                    if index_response['FaceRecords']:
                        for rec in index_response['FaceRecords']:
                            ...
                            sucesso += 1
                            resultados.append({'filename': nome_arquivo, 'external_image_id': nome_limpo, 'face_id': face_id, 'status': 'success'})
                    else:
                        falhas += 1
                        resultados.append({'filename': nome_arquivo, 'external_image_id': nome_limpo, 'face_id': '', 'status': 'no_face_detected'})
                except Exception as e:
                    falhas += 1
                    resultados.append({'filename': nome_arquivo, 'external_image_id': nome_limpo if 'nome_limpo' in locals() else '', 'face_id': '', 'status': f'error: {str(e)[:50]}'})

            return sucesso, puladas, falhas, resultados
        except Exception as e:
            raise Exception(f"Erro ao listar objetos no S3: {str(e)}")

    def buscar_rosto_por_imagem(self, image_bytes: bytes, collection_id: str = "meu_banco_de_rostos", threshold: float = 60.0, max_faces: int = 15) -> Tuple[bool, Optional[float], List[Dict]]:
        try:
            image_bytes = self.redimensionar_imagem(image_bytes)
            detect_response = self.rekognition.detect_faces(Image={'Bytes': image_bytes}, Attributes=['DEFAULT'])
    ...
    def buscar_rosto_s3(self, s3_key: str, collection_id: str = "meu_banco_de_rostos", threshold: float = 60.0, max_faces: int = 15) -> Tuple[bool, Optional[float], List[Dict]]:
        try:
