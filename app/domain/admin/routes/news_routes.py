...
@router.delete("/{event_id}/news/cache")
def clear_news_cache(event_id: int, user = Depends(require_subadmin_or_master)):
    """Limpa o cache Redis de todos os posts de um evento."""
    deleted = redis_client.delete_pattern(f"news:event:{event_id}:*")
    return {"cleared": deleted, "pattern": f"news:event:{event_id}:*"}
