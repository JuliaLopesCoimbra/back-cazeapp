...
class NewsService:
    ...
    def list_by_event(db, event_id: int, limit: int = 10, offset: int = 0, include_pending: bool = False):
        ...
        # Busca contagens de likes e comentários em batch (evita N+1)
        ...
        if post_ids:
            ...
            try:
                likes_counts = (
                    ...
                    .filter(Like.news_id.in_(post_ids), Like.is_active == True)
                    .group_by(Like.news_id)
                    .all()
                )
                likes_count_dict = {nid: cnt for nid, cnt in likes_counts}

                comments_counts = (
                    interaction_db.query(Comment.news_id, func.count(Comment.id).label("count"))
                    .filter(Comment.news_id.in_(post_ids), Comment.deleted_at.is_(None))
                    .group_by(Comment.news_id)
                    .all()
                )
                comments_count_dict = {nid: cnt for nid, cnt in comments_counts}
            finally:
        ...
        for post in posts:
            ...
            result.append({
                ...
                "status": post.status,
                "likes_count": likes_count_dict.get(post.id, 0),
                "comments_count": comments_count_dict.get(post.id, 0),
                "author": {
