...
class AuthRepository:
    ...
    @staticmethod
    def get_user_by_id_with_relations(db: Session, user_id: int):
    ...
    @staticmethod
    def list_subadmins(db: Session, search: Optional[str] = None, status: Optional[str] = None, limit: int = 50, offset: int = 0):
        """Lista subadmins com paginação, busca e filtro de status opcionais"""
        limit = min(limit, 100)
        query = db.query(User).options(
            joinedload(User.invited_by),
            joinedload(User.deactivated_by),
            joinedload(User.reactivated_by)
        ).filter(User.role == "subadmin")
        if search:
            query = query.filter(or_(User.name.ilike(f"%{search}%"), User.email.ilike(f"%{search}%")))
        if status:
            query = query.filter(User.status == status)
        return query.order_by(User.name.asc()).limit(limit).offset(offset).all()

    @staticmethod
    def list_colunistas(db: Session, invited_by_id: int = None, search: Optional[str] = None, status: Optional[str] = None, limit: int = 50, offset: int = 0):
        """Lista colunistas com paginação, busca e filtro de status opcionais"""
        limit = min(limit, 100)
        query = db.query(User).options(
            joinedload(User.invited_by),
            joinedload(User.deactivated_by),
    ...
    @staticmethod
    def list_users(db: Session, search: Optional[str] = None, status: Optional[str] = None, limit: int = 50, offset: int = 0):
        """Lista users comuns com paginação, busca e filtro de status opcionais"""
        limit = min(limit, 100)
        query = db.query(User).options(
            joinedload(User.invited_by),
            joinedload(User.deactivated_by),
