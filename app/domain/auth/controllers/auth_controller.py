...
class AuthController:
    ...
    def list_subadmins(db, requester, search: str = None, status: str = None, limit: int = 50, offset: int = 0):
        return AuthService.list_subadmins(db, requester, search, status, limit, offset)

    @staticmethod
    def list_colunistas(db, requester, search: str = None, status: str = None, limit: int = 50, offset: int = 0):
        return AuthService.list_colunistas(db, requester, search, status, limit, offset)

    @staticmethod
    def list_users(db, requester, search: str = None, status: str = None, limit: int = 50, offset: int = 0):
        return AuthService.list_users(db, requester, search, status, limit, offset)

    @staticmethod
    def get_user_detail(db, requester, user_id: int):
        return AuthService.get_user_detail(db, requester, user_id)

    @staticmethod
    def get_user_activity(db, interaction_db, admin_db, user_id: int, requester, limit: int = 30, offset: int = 0):
        return AuthService.get_user_activity(db, interaction_db, admin_db, user_id, requester, limit, offset)

    @staticmethod
    def resend_invite(db, user_id: int, requester):
        return AuthService.resend_invite(db, user_id, requester)
