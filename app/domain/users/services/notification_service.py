...
class NotificationService:
    ...
    def notify_new_post(notification_db, auth_db, admin_db, news_id: int, event_id: int):
        """Notifica todos os usuários sobre novo post via OneSignal (filtra por tag news_feed)."""
        from app.domain.admin.models.event_model import Event
        from app.infra.onesignal import send_for_category

        event = admin_db.query(Event).filter(Event.id == event_id).first()
        event_name = event.title if event else "evento"

        send_for_category(
            category_tag="news_feed",
    ...
    def notify_lineup_updated(notification_db, auth_db, admin_db, event_id: int):
        """Notifica quando o line up de um evento é atualizado via OneSignal (filtra por tag lineup_updated)."""
        from app.domain.admin.models.event_model import Event
        from app.infra.onesignal import send_for_category
    ...
    def notify_new_event(notification_db, auth_db, admin_db, event_id: int):
        """Notifica todos os usuários sobre um novo evento via OneSignal (filtra por tag new_events)."""
        from app.domain.admin.models.event_model import Event
        from app.infra.onesignal import send_for_category
        ...
        send_for_category(
            category_tag="new_events",
            title="Novo evento disponível!",
            message=f"Novo evento: {event.title}",
        )
    ...
    def restore_event_notifications(notification_db, auth_db, admin_db, event_id: int):
        """Notifica todos quando um evento é reativado via OneSignal (filtra por tag new_events)."""
        from app.domain.admin.models.event_model import Event
        from app.infra.onesignal import send_for_category
    ...
    def broadcast_notification(notification_db, auth_db, title: str, message: str, sender_id: int):
        """Envia notificação broadcast para todos os usuários via OneSignal."""
        from app.infra.onesignal import send_to_all

        excluded = NotificationPreferenceRepository.get_push_disabled_user_ids(notification_db)
        send_to_all(title=title, message=message, excluded_user_ids=excluded)
        return 0
