...
class LikeRepository:
    ...
    def create(db: Session, news_id: int, user_id: int, ip_address: str = None, user_agent: str = None, admin_db: Session = None):
        # Busca qualquer like existente (ativo ou inativo) para esta combinação
        ...
        if existing_like:
            if existing_like.is_active:
                # Já está curtido — retorna sem fazer nada
                return existing_like
            # Reativa o registro existente (evita UniqueViolation na constraint uq_likes_user_news)
            existing_like.is_active = True
            existing_like.deactivated_at = None
            existing_like.ip_address = ip_address
            existing_like.user_agent = user_agent
            db.commit()
