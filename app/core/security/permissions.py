...
class PermissionChecker:
    ...
    @staticmethod
    def is_promotor(user: User) -> bool:
    ...
    @staticmethod
    def can_scan_tshirt(user: User) -> bool:
        return user.role in ["admin_master", "subadmin", "promotor"]
...
def require_promotor_or_above(user: User = Depends(get_current_user)):
    """Admin_master, subadmin ou promotor podem acessar (para operações de camiseta)"""
    if not PermissionChecker.can_scan_tshirt(user):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Acesso permitido apenas para administradores e promotores."
        )
    return user
